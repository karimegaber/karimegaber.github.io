...
// dynamic and revalidate only work on Server Components, and we are using output: export.
// To bypass this build error but maintain CSR, we remove them. The isMounted state
// natively enforces true CSR and bypasses SSG.
...
export default function BlogPostPage(props: { params: Promise<{ slug: string }> }) {
  ...
  useEffect(() => {
    ...
    async function fetchPost() {
      // Do not fetch data for the dummy param created to satisfy the static build.
      if (!slug || slug === "placeholder-to-satisfy-export") {
          if (slug === "placeholder-to-satisfy-export") {
              setLoading(false);
          }
          return
      }
  ...
  // Mandatory Build-Time Lock / Loading State
